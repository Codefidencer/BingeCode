...
DevFlix mirrors any browser or media player window into a VS tool window using the Windows DWM Thumbnail API. It's hardware-accelerated, has zero CPU overhead, and does not use screen capture. It automatically detects and crops to the video-only area, stripping browser chrome, sidebars, and recommendations.

---

## Features

- **Mirror any window:** pick any visible browser or media player from the dropdown and the live DWM thumbnail appears instantly inside VS
...
- **Fullscreen mode:** F11 or double-click expands the mirror to fill the entire tool window and hides VS pane decorations, with the mini-bar fading in on mouse move
...
Install directly from the [Visual Studio Marketplace](https://marketplace.visualstudio.com/) and search for **DevFlix**.
...
| Pick a window from the dropdown | Starts mirroring, with video-only crop applied automatically |
...
DevFlix uses the **DWM Thumbnail API** (`DwmRegisterThumbnail`) to composite a live, hardware-accelerated copy of any window's client area directly onto the VS root HWND, with no screen capture or CPU rendering involved.
